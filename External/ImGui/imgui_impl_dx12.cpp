...
static void ImGui_ImplDX12_SetupRenderState(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, ImGui_ImplDX12_RenderBuffers* fr)
{
    ...
    // Our visible imgui space lies from draw_data->DisplayPos (top left_) to draw_data->DisplayPos+data_data->DisplaySize (bottom right_).
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Apply Scissor/clipping rectangle, Bind texture, DrawModel
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr, nullptr, "main", "vs_5_0", 0, 0, &vertexShaderBlob, nullptr)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Finalize() the blob!
